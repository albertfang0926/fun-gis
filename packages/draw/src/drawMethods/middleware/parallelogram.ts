...
// customs
import { coordinateToCartesian3, cartesian3ToCoordinate, getParallelogramVertices } from "../utils"
import { drawParallelogram } from "../core"
...
class Parallelogram {
  ...
  public draw(): void {
    drawParallelogram(this.viewer, {}, (e) => {
  ...
  onDrag() {
    this.dragHandler.setInputAction((e) => {
      ...
      if (
        ...
        const previousTwo = (4 + (focusedIndex - 2)) % 4
        const previousOne = (4 + (focusedIndex - 1)) % 4
        const previousOnePoint = this.controlPoints[previousOne]
        const previousTwoPoint = this.controlPoints[previousTwo]
        ...
        this.dragHandler.setInputAction((e: any) => {
          ...
          if (position) {
            ...
            //   id: pickedControlPoint.primitive.id,
            //   show: true,
            //   position: position,
            ...
            this.controlPoints.forEach((controlPoint, index) => {
              const h = helperPrimitives.add({
                ...
                show: true,
                position: controlPoint,
                image: focusedIndex === index ? highlightHelpPoint : helpPoint,
                scale: focusedIndex === index ? 0.5 : 0.3,
                verticalOrigin: VerticalOrigin.CENTER,
                scaleByDistance: new NearFarScalar(1.5e2, 1.5, 8.0e6, 0.0),
                disableDepthTestDistance: Number.POSITIVE_INFINITY,
                heightReference: HeightReference.NONE
