...
// type  "arc":"弧形"    "arch":"弓形"   "sector":扇形"
const drawSector = (
  viewer: Viewer,
  options: Record<string, any>,
  type: "arc" | "arch" | "sector",
  callback: (e: any) => void,
  cancelCallback?: () => void
) => {
  // 解析参数
  ...
  // 操作提示文本
  const toolTipText = {
    start: "单击确定圆心</br>右键取消绘制",
    process: "单击确定弧形起点</br>右键取消绘制",
   ...
   * 获取插值后的positions
   */
  const getPositions = (positions: Coordinate[]) => {
    if (type === "arc") {
      return calculateArcPoints(positions)
    } else if (type === "arch") {
      return calculateArchPoints(positions)
    } else {
      return calculateSectorPoints(positions)
    }
  }
  ...
  _handler.setInputAction((move: ScreenSpaceEventHandler.MotionEvent) => {
    ...
    if (cLength === 1) {
      tooltip.showAt(move.endPosition, toolTipText.process)
      return
    }
    // 计算鼠标位置处的坐标
    ...
    tempLineCoordinates[tempLineCoordinates.length - 1] = cartesian3ToCoordinate(
      tempLinePositions[tempLinePositions.length - 1],
      viewer
    )
    // tempLinePositions = getAttackArrowPoints(tempLineCoordinates)
