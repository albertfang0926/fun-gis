...
const drawSubmarineState = (
  ...
  // 记录临时坐标
  let tempCirclePoints: Cartesian3[] = []
  ...
  const getPrimitive = (
    ...
    // 添加自定义图形及线
    const lineGeometryInstances: GeometryInstance[] = []
    linePositions.forEach((item) => {
      lineGeometryInstances.push(
        new GeometryInstance({
  ...
  const _preUpdateHandler = () => {
    if (changedFlag) {
      changedFlag = false
      const primitive = getPrimitive(tempCirclePoints, tempLinePositions)
...
export default drawSubmarineState
